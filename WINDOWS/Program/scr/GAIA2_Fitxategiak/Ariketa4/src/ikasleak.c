...
void MENUA_erakutsi() {
	...
	do {
		...
		printf("10. Ikasle baten informazio guztia fitxategian idatzi\n");
		printf("11. Ikasle guztien informazioa fitxategian idatzi\n");
		printf("12. Fitxategitik ikasle guztien informazioa irakurri\n");
...
int MENUA_exekutatu(int aukera, IKASLE ikasleak[], int zenbat){
	...
	case BISTARATU_FINALA:
		...
		break;
	case IKASLEA_BAT_FITXATEGI:

		fitxategi_ikasle(ikasleak, zenbat);

		break;
	case IKASLE_GUZTIAK_FITXATEGI:

		fitxategi_ikasle_guztiak(ikasleak, zenbat);

		break;	
	case IRAKURRI_FIXTATEGI:

		fitxategi_ikasle_irakurri(ikasleak, zenbat);
...
void fitxategi_ikasle(IKASLE ikasleak[], int zenbat) {
	...
	ikasleNotaBat = fopen(IKASLEA_BATEN_NOTA, "a");
	...
	if (ikasleNotaBat > 0) {
		...
		if (zenbat <= 0) {
			...
			while (i < zenbat && aurkitu == 0) {
				if (id == ikasleak[i].id) {
					aurkitu = 1;
					fprintf(ikasleNotaBat,"%i\t%s\t%s\t otsaileko lehenego nota %f\n\t\t\t otsaileko bigarren nota %f\n\t\t\t ekaineko lehenego nota %f\n\t\t\t ekaineko lehenego nota %f\n\t\t\t finala %f", ikasleak[i].id, ikasleak[i].izena, ikasleak[i].genero, ikasleak[i].partziNota.otsaila.exam1, ikasleak[i].partziNota.otsaila.exam2, ikasleak[i].partziNota.ekaina.exam1, ikasleak[i].partziNota.ekaina.exam2, ikasleak[i].final);
				}
				i++;
			}
			if (aurkitu == 0) {
...
void fitxategi_ikasle_guztiak(IKASLE ikasleak[], int zenbat) {
	...
	FILE* ikasleNotaGuztia;

	int i = 0;
	char str[128];

	ikasleNotaGuztia = fopen(IKASLE_GUZTIEN_NOTA, "a");


	if (ikasleNotaGuztia > 0) {
		...
		if (zenbat <= 0) {
			...
			for (i = 0; i < zenbat; i++) {
				fprintf(ikasleNotaGuztia, "%i\t%s\t%s\t otsaileko lehenego nota %f\n\t\t\t otsaileko bigarren nota %f\n\t\t\t ekaineko lehenego nota %f\n\t\t\t ekaineko lehenego nota %f\n\t\t\t finala %f\n", ikasleak[i].id, ikasleak[i].izena, ikasleak[i].genero, ikasleak[i].partziNota.otsaila.exam1, ikasleak[i].partziNota.otsaila.exam2, ikasleak[i].partziNota.ekaina.exam1, ikasleak[i].partziNota.ekaina.exam2, ikasleak[i].final);
			}
			printf("Egina\n");
		}

		fclose(ikasleNotaGuztia);
	}
	else {
		printf("ERROR: Arazoak %s fitxategia zabaltzerakoan.\n", ikasleNotaGuztia);
	}
...
void fitxategi_ikasle_irakurri(IKASLE ikasleak[], int zenbat) {
	...
	FILE* ikasleNotaIrakurri;

	int i = 0;
	char str[128];
	IKASLE irakurri[100];

	ikasleNotaIrakurri = fopen(IKASLE_IRAKURRI, "r");
	int egoera;
	if (ikasleNotaIrakurri > 0) {
		...
		if (zenbat <= 0) {
			...
			for (i = 0; i < zenbat; i++) {
				...
				egoera = fscanf(ikasleNotaIrakurri, "%i%s%s%f%f%f%f%f\n", &ikasleak[i].id, &ikasleak[i].izena, &ikasleak[i].genero, &ikasleak[i].partziNota.otsaila.exam1, ikasleak[i].partziNota.otsaila.exam2, &ikasleak[i].partziNota.ekaina.exam1, &ikasleak[i].partziNota.ekaina.exam2, &ikasleak[i].final);
				
			}
			
		for (i = 0; i < zenbat; i++) {					
			printf("%i\t%s\t%s\t otsaileko lehenego nota %f\n\t\t\t otsaileko bigarren nota %f\n\t\t\t ekaineko lehenego nota %f\n\t\t\t ekaineko lehenego nota %f\n\t\t\t finala %f\n", ikasleak[i].id, ikasleak[i].izena, ikasleak[i].genero, ikasleak[i].partziNota.otsaila.exam1, ikasleak[i].partziNota.otsaila.exam2, ikasleak[i].partziNota.ekaina.exam1, ikasleak[i].partziNota.ekaina.exam2, ikasleak[i].final);	}
			printf("Egina\n");
		}

		fclose(ikasleNotaIrakurri);
	}
	else {
		printf("ERROR: Arazoak %s fitxategia zabaltzerakoan.\n", ikasleNotaIrakurri);
	}
