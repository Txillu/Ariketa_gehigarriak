...
void MENUA_erakutsi() {
	...
	do {
		...
		printf("10. Ikasle baten informazio guztia fitxategian idatzi BITARREAN\n");
		printf("11. Ikasle guztien informazioa fitxategian idatzi BITARREAN\n");
		printf("12. Fitxategitik ikasle guztien informazioa irakurri BITARREAN\n");
	...
	if (ikasleNotaBat > 0) {
		...
		if (zenbat <= 0) {
			...
			while (i < zenbat && aurkitu == 0) {
				if (id == ikasleak[i].id) {
					aurkitu = 1;
					fwrite(ikasleak, sizeof(IKASLE), zenbat, ikasleNotaBat);
				}
				i++;
			}
			if (aurkitu == 0) {
...
void fitxategi_ikasle_guztiak(IKASLE ikasleak[], int zenbat) {
	...
	if (ikasleNotaGuztia > 0) {
		...
		if (zenbat <= 0) {
			...
			for (i = 0; i < zenbat; i++) {
				fwrite(ikasleak, sizeof(IKASLE), zenbat, ikasleNotaGuztia);
			}
			printf("Egina\n");
		}
	...
	if (ikasleNotaIrakurri > 0) {
		...
		if (zenbat <= 0) {
			...
			for (i = 0; i < zenbat; i++) {
				...
				egoera = fread(ikasleak, sizeof(IKASLE), zenbat, ikasleNotaIrakurri);
